    ...
    public sealed class MemoryRoomBootstrap : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            var hudController = new MemoryRoomHudController(
                hudView, session.Trust, session.ExtractionBudget, session.Wallet, session.EventBus);
            ...
            // 마스크는 이제 UI Toolkit 요소다(같은 root 안의 mask-left/right).
            // 카메라가 흔들려도 프레임처럼 고정되게 하려는 것.
            var maskView = new MemoryRoomMaskView(root);
            var maskController = new MemoryRoomMaskController(
                maskView, session.Visibility, session.Trust, session.EventBus);
            ...
            var dialogueController = new DialoguePanelController(
                dialogueView, session.Dialogue, session.CensorUnlock, session.Wallet,
                session.CensorKeyColors, session.EventBus);
            ...
            var clueUseController = new ClueUsePanelController(
                clueUseView, session.ExtractionProcessor, session.ClueState,
                session.ExtractionBudget, session.EventBus);
            ...
            _screenController = new MemoryRoomScreenController(
                ...
                restorationController, overlayTabs, _overlayPanels);
